  ...
  return (
    <div className='w-full flex flex-col justify-end items-end gap-2 p-2'>
      {fields.map((feeTypeForm, index) => {
        return (
          <div className='w-full bg-gray-100 rounded-lg p-2 border border-gray-300' key={feeTypeForm.id}>
            <Form {...form}>
              <div  className="">
...
function FeeStructureFormYearwise({ index, form }: { index: number, form: UseFormReturn<z.infer<typeof FormSchema>> }) {
  ...
  return (
    <div className='w-full flex flex-col items-center justify-center border-2 border-dotted border-gray-300 rounded-lg p-2'>
      {/* <SelectRegdYear onSubmit={onRegdYrSubmit} loading={loading} /> */}
      <Form {...form}>
        <div className="space-y-2">
          {fields.map((field, fsyIndex) => (
            <div className='grid grid-cols-7 gap-2' key={field.id}>
              {/* Description Field */}
          ...
          <Button type="button" variant={'outline'} onClick={() => append({ description: '', amount: '', payType: 'YEARLY', tfwType: 'TFW', comments: '', tatFees: 0, tactFees: 0 })}>
            Add Fees
          </Button>
        </div>
      </Form>
...
export default function FeeStructureGenerationForm({ token }: { token: string }) {
  ...
  return (
    <div className='w-3/4 flex flex-col items-center justify-center border shadow-lg rounded-lg p-2'>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full bg-green-50 rounded-lg space-y-5 flex flex-col justify-center items-center">
