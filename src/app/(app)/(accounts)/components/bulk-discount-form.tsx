...
import { handleBulkDiscountUpload } from '../server-actions-fee-collection/actions';
import { Loader } from 'lucide-react';

// Function to download the CSV template
const downloadCsvTemplate = () => {
  const headers = ['regdNo', 'particulars', 'discount'];
  ...
  link.setAttribute(
    'download',
    `bulk_discount_template-${new Date().toISOString()}.csv`,
  );
...
const BulkDiscountForm = () => {
      ...
      try {
        ...
        Papa.parse(file, {
          ...
          transform: (value, field: string) => {
            const fieldsToUppercase = ['particulars'];
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        const response = (await handleBulkDiscountUpload(formData)) as Resp;
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center items-center gap-3">
      <h1 className="text-xl">Bulk Discount Form</h1>
        ...
        <li>
          ...
          <div className="border rounded-lg p-2 bg-red-500">
            <ul className="list-disc px-4 text-white">
              <li>
                Enter valid <b>Registration</b> number
              </li>
              <li>
                Enter valid <b>Particulars</b> header
              </li>
            </ul>
          </div>
        </li>
