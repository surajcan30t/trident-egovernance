...
const FeeCreationSchema = z.object({
  ...
  type: z.enum(
    ['COMPULSORY_FEES', 'OPTIONAL_FEES', 'EXCESS_FEE', 'OTHER_FEES', ''],
    {
  ...
  deductionOrder: z.string(),
});

const formSchema = z.object({
  newFees: z
    .array(FeeCreationSchema, {
  ...
  return (
    <>
      {fields.map((formSchema, index) => {
        return (
          <div
            className="w-full rounded-lg p-2 border border-gray-500 mb-5"
            key={formSchema.id}
          >
            <Form {...form}>
              <form className="w-full grid grid-rows-2 gap-2 border rounded-lg p-2 shadow-lg">
                <div className="gap-2 grid grid-cols-7">
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="COMPULSORY_FEES">
                                COMPULSORY_FEES
                              </SelectItem>
                              <SelectItem value="OPTIONAL_FEES">
                                OPTIONAL_FEES
                              </SelectItem>
                              <SelectItem value="EXCESS_FEE">
                                EXCESS_FEE
                              </SelectItem>
                              <SelectItem value="OTHER_FEES">
                                OTHER_FEES
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        ...
                        <FormControl>
                          <div className="flex flex-row">
                            <Input
                              ...
                              placeholder=""
                              {...field}
                              list="feeGroupOptions"
                            />
                            <datalist id="feeGroupOptions">
                              {feeGroups.map((feeGroup) => (
                                <option key={feeGroup} value={feeGroup}>
                                  {feeGroup}
                                </option>
                              ))}
                            </datalist>
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <div className="flex flex-row">
                            <Input
                              className="uppercase"
                              placeholder="e.g. TRANSPORTFEES"
                              {...field}
                              list="partOfOptions"
                            />
                            <datalist id="partOfOptions">
                              {partOf.map((partOf) => (
                                <option key={partOf} value={partOf}>
                                  {partOf}
                                </option>
                              ))}
                            </datalist>
                  ...
                  <FormField
                    control={form.control}
                    name={`newFees.${index}.deductionOrder`}
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Deduction Order</FormLabel>
                        <FormControl>
                          <Input placeholder="e.g. 1" {...field} />
                        </FormControl>
                        <FormMessage />
    ...
    <>
      <Form {...form}>
        <form
          ...
          className="w-full grid grid-cols-1 gap-2 border rounded-lg p-2 shadow-lg"
        >
          <div className="gap-2">
          ...
          <div className="flex justify-end items-center p-4">
            <Button
              variant="trident"
              disabled={form.watch()?.newFees?.length === 0}
              type="submit"
            >
              {loading ? (
                <Loader
